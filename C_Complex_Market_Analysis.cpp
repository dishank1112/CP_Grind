...
vector<bool> generatePrimeSieve(int limit)
{
...
void solve() {
    ...
    int LIMIT = 1000001;
    static vector<bool> isPrime = generatePrimeSieve(LIMIT);
    ll ans = 0;
    for(ll i=0;i<e;i++){
        vector<ll>grp;
        for(ll start=i;start<n;start+=e){
            grp.push_back(arr[start]);
        }
        ll m = grp.size();
        vector<ll>left(m,0);      
        vector<ll>right(m,0);
        for(ll ind=0;ind<m;ind++){
            if(ind == 0){
                if(grp[ind] == 1)left[ind]=1;
            }
            else if(ind>0 && grp[ind] == 1){
                left[ind] = left[ind-1]+1;
            }
        }
        for (ll ind = m-1; ind >= 0; ind--)
        {
            if (ind == m-1)
            {
                if (grp[ind] == 1)
            ...
            {
                right[ind] = right[ind + 1] + 1;
            }
        }
        for(ll i=0;i<m;i++){
          if(isPrime[grp[i]]){
            ll leftcnt = (i>0 ? left[i-1] : 0);
            ll rightcnt  = (i<m-1 ? right[i+1] : 0);
            ans += (leftcnt+1)*(rightcnt+1)-1;
          }
        }
    }
    cout<<ans<<endl;
